"""
https://github.com/deepinsight/insightface/blob/c0b95cc66f2fb7ea2a492560f017aefb0384d32b/recognition/arcface_mxnet/image_iter.py

Modified by Jean-Remy Conti
"""
	...
	def next(self):
		...
		try:
			while i < batch_size:
				...
				if len(s) == 0:																	# Little change (bug if dataset length is not multiple of batch size)
					return io.DataBatch([batch_data[:i]], [batch_label[:i]], batch_size - i+1)	# -> the last batch is not complete
				else:
				...
				for datum in data:
					...
					batch_label[i][:] = label [0] 												# Change to label instead of label[0] if problem of dimension
					i += 1
